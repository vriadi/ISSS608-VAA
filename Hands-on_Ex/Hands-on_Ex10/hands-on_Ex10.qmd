---
title: "Information Dashboard Design: R methods"
author: "Vanessa Riadi"
date: "Jun 20, 2025"
date-modified: "last-modified"
format:
  html:
    toc: true
execute: 
  echo: true
  eval: true
  warning: false
  freeze: true
---

## [1]{style="color:#d496d4; background:none;"} Overview

By the end of this hands-on exercise, you will be able to:

-   create bullet chart by using ggplot2,
-   create sparklines by using ggplot2 ,
-   build industry standard dashboard by using R Shiny.

## [2]{style="color:#d496d4; background:none;"} Getting started

R packages used in this chapter:

-   [tidyverse](https://www.tidyverse.org/) provides a collection of functions for performing data science task such as importing, tidying, wrangling data and visualising data. It is not a single package but a collection of modern R packages including but not limited to [readr](https://readr.tidyverse.org/), [tidyr](https://tidyr.tidyverse.org/), [dplyr](https://dplyr.tidyverse.org/), [ggplot](https://ggplot2.tidyverse.org/), [tibble](https://tibble.tidyverse.org/), [stringr](https://stringr.tidyverse.org/), [forcats](https://forcats.tidyverse.org/) and [purrr](https://purrr.tidyverse.org/).
-   [lubridate]() provides functions to work with dates and times more efficiently.
-   [ggthemes]() is an extension of ggplot2. It provides additional [themes](https://yutannihilation.github.io/allYourFigureAreBelongToUs/ggthemes/) beyond the basic themes of ggplot2.
-   [gtExtras](https://jthomasmock.github.io/gtExtras/index.html) provides some additional helper functions to assist in creating beautiful tables with [gt](https://gt.rstudio.com/), an R package specially designed for anyone to make wonderful-looking tables using the R programming language.
-   [reactable](https://glin.github.io/reactable/index.html) provides functions to create interactive data tables for R, based on the [React Table](https://github.com/tanstack/table/tree/v7) library and made with [reactR](https://github.com/react-R/reactR).
-   [reactablefmtr](https://kcuilla.github.io/reactablefmtr/index.html) provides various features to streamline and enhance the styling of interactive reactable tables with easy-to-use and highly-customizable functions and themes.

Load the R Packages
```{r}
#| code-fold: true
pacman::p_load(lubridate, ggthemes, reactable,
reactablefmtr, gt, gtExtras, tidyverse,odbc,svglite,ggplot2,knitr)
```

## [3]{style="color:#d496d4; background:none;"} Importing Microsoft Access database

### [3.1]{style="color:#d496d4; background:none;"} The data set

A personal database in Microsoft Access mdb format called *Coffee Chain* will be used.

### [3.2]{style="color:#d496d4; background:none;"} Importing database into R (Only work for R system 32 bit)

In the code chunk below, `odbcConnectAccess()` of [**RODBC**](https://cran.r-project.org/web/packages/RODBC/index.html) package is used used to import a database query table into R.

```{r}
#| eval: false
con <- odbcConnectAccess2007('../data/Coffee Chain.mdb')
coffeechain <- sqlFetch(con, 'CoffeeChain Query')
write_rds(coffeechain, "../data/CoffeeChain.rds")
odbcClose(con)
```
::: {.nursebox .nurse data-latex="nurse"}
Before running the code chunk, you need to change the R system to 32bit version. This is because the `odbcConnectAccess()` is based on 32bit and not 64bit. However, macOS the latest version no longer support 32-bit. We wil skip running this code, and directly use the dataset provided in E-Learn by Prof Kam.
:::

### [3.3]{style="color:#d496d4; background:none;"} Data Preparation

The code chunk below is used to import *CoffeeChain.rds* into R.

```{r}
coffeechain <- read_rds("../data/rds/CoffeeChain.rds")
```

```{r}
kable(head(coffeechain, n=5))
```
The code chunk below is used to aggregate Sales and Budgeted Sales at the Product level.

```{r}
#| code-fold: true
product <- coffeechain %>%
  group_by(`Product`) %>%
  summarise(`target` = sum(`Budget Sales`),
            `current` = sum(`Sales`)) %>%
  ungroup()
```

### [3.4]{style="color:#d496d4; background:none;"} Bullet chart in ggplot2

The code chunk below is used to plot the bullet charts using ggplot2 functions.

```{r}
#| code-fold: true
bulletcharts <- ggplot(product, aes(Product, current)) + 
  geom_col(aes(Product, max(target) * 1.01),
           fill="grey85", width=0.85) +
  geom_col(aes(Product, target * 0.75),
           fill="grey60", width=0.85) +
  geom_col(aes(Product, target * 0.5),
           fill="grey50", width=0.85) +
  geom_col(aes(Product, current), 
           width=0.35,
           fill = "black") + 
  geom_errorbar(aes(y = target,
                    x = Product, 
                    ymin = target,
                    ymax= target), 
                width = .4,
                colour = "red",
                linewidth = 1) +
  coord_flip()

bulletcharts
```

## [4]{style="color:#d496d4; background:none;"} Plotting sparklines using ggplot2

In this section, you will learn how to plot sparklines by using ggplot2.

### [4.1]{style="color:#d496d4; background:none;"} Preparing the data

```{r}
#| eval: false
#| code-fold: true
sales_report <- coffeechain %>%
  filter(Date >= "2013-01-01") %>%
  mutate(Month = month(Date)) %>%
  group_by(Month, Product) %>%
  summarise(Sales = sum(Sales)) %>%
  ungroup() %>%
  select(Month, Product, Sales)
```
```{r}
sales_report <- coffeechain %>% 
  filter(Date >= "2013-01-01") %>% 
  mutate(Month = month(Date)) %>% 
  group_by(Month, Product) %>% 
  summarise(
    Sales   = sum(Sales),
    .groups = "drop"        # or "drop_last" if you prefer
  ) %>% 
  select(Month, Product, Sales)
```

The code chunk below is used to compute the minimum, maximum and end othe the month sales.

```{r}
#| code-fold: true
mins <- group_by(sales_report, Product) %>% 
  slice(which.min(Sales))
maxs <- group_by(sales_report, Product) %>% 
  slice(which.max(Sales))
ends <- group_by(sales_report, Product) %>% 
  filter(Month == max(Month))

```

The code chunk below is used to compute the 25 and 75 quantiles.

```{r}
#| code-fold: true
quarts <- sales_report %>%
  group_by(Product) %>%
  summarise(quart1 = quantile(Sales, 
                              0.25),
            quart2 = quantile(Sales, 
                              0.75)) %>%
  right_join(sales_report)

```

### [4.2]{style="color:#d496d4; background:none;"} sparklines in ggplot2

The code chunk used.

```{r}
#| code-fold: true
#| fig-width: 14
#| fig-height: 12
sparklines <- ggplot(sales_report, aes(x=Month, y=Sales)) + 
  facet_grid(Product ~ ., scales = "free_y") + 
  geom_ribbon(
  data = quarts,
  aes(
    x     = Month,          # ← tell ggplot what the ribbon’s x is
    ymin  = quart1,
    ymax  = quart2
  ),
  fill = "grey90",
  inherit.aes = FALSE        # keep this OFF so the layer gets only what we pass
) +
  geom_line(size=0.3) +
  geom_point(data = mins, col = 'red') +
  geom_point(data = maxs, col = 'blue') +
  geom_text(data = mins, aes(label = Sales), vjust = -1) +
  geom_text(data = maxs, aes(label = Sales), vjust = 2.5) +
  geom_text(data = ends, aes(label = Sales), hjust = 0, nudge_x = 0.5) +
  geom_text(data = ends, aes(label = Product), hjust = 0, nudge_x = 1.0) +
  expand_limits(x = max(sales_report$Month) + 
                  (0.25 * (max(sales_report$Month) - min(sales_report$Month)))) +
  scale_x_continuous(breaks = seq(1, 12, 1)) +
  scale_y_continuous(expand = c(0.1, 0)) +
  theme_tufte(base_size = 3, base_family = "Helvetica") +
  theme(axis.title=element_blank(), axis.text.y = element_blank(), 
        axis.ticks = element_blank(), strip.text = element_blank())

sparklines
```

## [5]{style="color:#d496d4; background:none;"} Static Information Dashboard Design: gt and gtExtras methods

In this section, you will learn how to create static information dashboard by using [gt](https://gt.rstudio.com/index.html) and [gtExtras](https://jthomasmock.github.io/gtExtras/index.html) packages. Before getting started, it is highly recommended for you to visit the webpage of these two packages and review all the materials provided on the webpages at least once. You done not have to understand and remember everything provided but at least have an overview of the purposes and functions provided by them.

### [5.1]{style="color:#d496d4; background:none;"} Plotting a simple bullet chart

In this section, you will learn how to prepare a bullet chart report by using functions of gt and gtExtras packages.

```{r}
#| code-fold: true
product %>%
  gt::gt() %>%
  gt_plt_bullet(column = current, 
              target = target, 
              width = 60,
              palette = c("lightblue", 
                          "black")) %>%
  gt_theme_538()
```

## [6]{style="color:#d496d4; background:none;"} sparklines: gtExtras method

Before we can prepare the sales report by product by using gtExtras functions, code chunk below will be used to prepare the data.

```{r}
#| code-fold: true
report <- coffeechain %>%
  mutate(Year = year(Date)) %>%
  filter(Year == "2013") %>%
  mutate (Month = month(Date, 
                        label = TRUE, 
                        abbr = TRUE)) %>%
  group_by(Product, Month) %>%
  summarise(Sales = sum(Sales)) %>%
  ungroup()

```

It is important to note that one of the requirement of gtExtras functions is that almost exclusively they require you to pass data.frame with list columns. In view of this, code chunk below will be used to convert the report data.frame into list columns.

```{r}
#| code-fold: true
report %>%
  group_by(Product) %>%
  summarize('Monthly Sales' = list(Sales), 
            .groups = "drop")
```

### [6.1]{style="color:#d496d4; background:none;"} Plotting Coffechain Sales report

```{r}
#| code-fold: true
report %>%
  group_by(Product) %>%
  summarize('Monthly Sales' = list(Sales), 
            .groups = "drop") %>%
   gt() %>%
   gt_plt_sparkline('Monthly Sales',
                    same_limit = FALSE)
```

### [6.2]{style="color:#d496d4; background:none;"} Adding statistics

First, calculate summary statistics by using the code chunk below.

```{r}
#| code-fold: true
report %>% 
  group_by(Product) %>% 
  summarise("Min" = min(Sales, na.rm = T),
            "Max" = max(Sales, na.rm = T),
            "Average" = mean(Sales, na.rm = T)
            ) %>%
  gt() %>%
  fmt_number(columns = 4,
    decimals = 2)
```

### [6.3]{style="color:#d496d4; background:none;"} Combining the data.frame

Next, use the code chunk below to add the statistics on the table.

```{r}
#| code-fold: true
spark <- report %>%
  group_by(Product) %>%
  summarize('Monthly Sales' = list(Sales), 
            .groups = "drop")

sales <- report %>% 
  group_by(Product) %>% 
  summarise("Min" = min(Sales, na.rm = T),
            "Max" = max(Sales, na.rm = T),
            "Average" = mean(Sales, na.rm = T)
            )

sales_data = left_join(sales, spark)
```

### [6.4]{style="color:#d496d4; background:none;"} Plotting the updated data.table

```{r}
#| code-fold: true
sales_data %>%
  gt() %>%
  gt_plt_sparkline('Monthly Sales',
                   same_limit = FALSE)
```

### [6.5]{style="color:#d496d4; background:none;"} Combining bullet chart and sparklines

Similarly, we can combining the bullet chart and sparklines using the steps below.

```{r}
#| code-fold: true
bullet <- coffeechain %>%
  filter(Date >= "2013-01-01") %>%
  group_by(`Product`) %>%
  summarise(`Target` = sum(`Budget Sales`),
            `Actual` = sum(`Sales`)) %>%
  ungroup() 

sales_data = sales_data %>%
  left_join(bullet)

combined_charts <- sales_data %>%
  gt() %>%
  gt_plt_sparkline('Monthly Sales') %>%
  gt_plt_bullet(column = Actual, 
                target = Target, 
                width = 28,
                palette = c("lightblue", 
                          "black")) %>%
  gt_theme_538()

combined_charts
```

## [7]{style="color:#d496d4; background:none;"} Interactive Information Dashboard Design: reactable and reactablefmtr methods

In this section, you will learn how to create interactive information dashboard by using [reactable](https://glin.github.io/reactable/index.html) and [reactablefmtr](https://kcuilla.github.io/reactablefmtr/index.html) packages. Before getting started, it is highly recommended for you to visit the webpage of these two packages and review all the materials provided on the webpages at least once. You done not have to understand and remember everything provided but at least have an overview of the purposes and functions provided by them.

In order to build an interactive sparklines, we need to install dataui R package by using the code chunk below.

```{r}
#| eval: false
remotes::install_github("timelyportfolio/dataui")
```

Next, you all need to load the package onto R environment by using the code chunk below.

```{r}
library(dataui)
```

### [7.1]{style="color:#d496d4; background:none;"} Plotting interactive sparklines

Similar to gtExtras, to plot an interactive sparklines by using reactablefmtr package we need to prepare the list field by using the code chunk below.

```{r}
#| code-fold: true
report <- report %>%
  group_by(Product) %>%
  summarize(`Monthly Sales` = list(Sales))
```

Next, [react_sparkline](https://kcuilla.github.io/reactablefmtr/reference/react_sparkline.html) will be to plot the sparklines as shown below.

```{r}
#| code-fold: true
reactable(
  report,
  columns = list(
    Product = colDef(maxWidth = 200),
    `Monthly Sales` = colDef(
      cell = react_sparkline(report)
    )
  )
)
```

### [7.2]{style="color:#d496d4; background:none;"} Changing the pagesize

By default the pagesize is 10. In the code chunk below, arguments defaultPageSize is used to change the default setting.

```{r}
#| code-fold: true
reactable(
  report,
  defaultPageSize = 13,
  columns = list(
    Product = colDef(maxWidth = 200),
    `Monthly Sales` = colDef(
      cell = react_sparkline(report)
    )
  )
)
```

### [7.3]{style="color:#d496d4; background:none;"} Adding points and labels

In the code chunk below `highlight_points` argument is used to show the minimum and maximum values points and `label` argument is used to label first and last values.

```{r}
#| code-fold: true
reactable(
  report,
  defaultPageSize = 13,
  columns = list(
    Product = colDef(maxWidth = 200),
    `Monthly Sales` = colDef(
      cell = react_sparkline(
        report,
        highlight_points = highlight_points(
          min = "red", max = "blue"),
        labels = c("first", "last")
        )
    )
  )
)
```

### [7.4]{style="color:#d496d4; background:none;"} Adding reference line

In the code chunk below `statline` argument is used to show the mean line.

```{r}
#| code-fold: true
reactable(
  report,
  defaultPageSize = 13,
  columns = list(
    Product = colDef(maxWidth = 200),
    `Monthly Sales` = colDef(
      cell = react_sparkline(
        report,
        highlight_points = highlight_points(
          min = "red", max = "blue"),
        statline = "mean"
        )
    )
  )
)
```

### [7.5]{style="color:#d496d4; background:none;"} Adding bandline

Instead adding reference line, bandline can be added by using the `bandline` argument.

```{r}
#| code-fold: true
reactable(
  report,
  defaultPageSize = 13,
  columns = list(
    Product = colDef(maxWidth = 200),
    `Monthly Sales` = colDef(
      cell = react_sparkline(
        report,
        highlight_points = highlight_points(
          min = "red", max = "blue"),
        line_width = 1,
        bandline = "innerquartiles",
        bandline_color = "green"
        )
    )
  )
)
```

### [7.6]{style="color:#d496d4; background:none;"} Changing from sparkline to sparkbar

Instead of displaying the values as sparklines, we can display them as sparkbars as shown below.

```{r}
#| code-fold: true
sparkbar <- reactable(
  report,
  defaultPageSize = 13,
  columns = list(
    Product = colDef(maxWidth = 200),
    `Monthly Sales` = colDef(
      cell = react_sparkbar(
        report,
        highlight_bars = highlight_bars(
          min = "red", max = "blue"),
        bandline = "innerquartiles",
        statline = "mean")
    )
  )
)

sparkbar
```

## 🎀 [PRACTICE 1]{style="color:#d496d4"}
```{r}
#| code-fold: true
combined_charts2 <- sales_data %>%
  gt() %>%
  gt_plt_sparkline('Monthly Sales') %>%
  gt_plt_bullet(column = Actual, 
                target = Target, 
                width = 28,
                palette = c("lightblue", "red")) %>%
  tab_header(
    title = md("**Coffeechain Sales Dashboard**"),
    subtitle = md("Monthly Sales Trends with Target Comparison")
  ) %>%
  gt_theme_538() %>%
  tab_options(
    table.border.top.color = "white",
    table.border.bottom.color = "white",
    table.font.size = 14,
    heading.title.font.size = 18,
    heading.subtitle.font.size = 14,
    row.striping.background_color = "grey98"
  ) 

combined_charts2

```

## 🎀 [PRACTICE 2]{style="color:#d496d4"}
```{r}
#| code-fold: true
profit_summary <- coffeechain %>%
  group_by(Market, `Product Type`) %>%
  summarise(
    Actual_Profit2 = sum(Profit),
    Budget_Profit2  = sum(`Budget Profit`),
    Actual_Profit  = sum(Profit),
    Budget_Profit  = sum(`Budget Profit`),
    Profit_Diff    = Actual_Profit - Budget_Profit,
    .groups = "drop"
  ) %>%
  arrange(desc(Profit_Diff))

gt_profit_dashboard <- profit_summary %>%
  gt() %>%
  # bar shows *direction* and *size* of gap
  gt_plt_bullet(column = Actual_Profit, 
                target = Budget_Profit, 
                width = 28,
                palette = c("lightblue", "red")) %>%
  cols_label(
    Market         = "Market",
    `Product Type` = "Product Type",
    Actual_Profit  = "Actual vs Budget Profit",
    Actual_Profit2  = "Actual Profit",
    Budget_Profit2  = "Budget Profit",
    Budget_Profit  = "Budget Profit",
    Profit_Diff    = "Δ Profit"
  ) %>%
  fmt_number(
    columns = c(Actual_Profit2, Budget_Profit2, Actual_Profit, Budget_Profit, Profit_Diff),
    decimals = 0, use_seps = TRUE
  ) %>%
  tab_header(
    title    = md("Profit Performance Dashboard"),
    subtitle = md("Actual vs Budget Profit by Market and Product Type")
  ) %>%
  gt_theme_538() %>%
  tab_options(
    table.font.size          = 14,
    heading.title.font.size  = 18,
    heading.subtitle.font.size = 13,
    row.striping.background_color = "grey98"
  )

gt_profit_dashboard


```


## [8]{style="color:#d496d4; background:none;"} Reference

-   [Lesson Slides](https://isss608-ay2024-25apr.netlify.app/lesson/lesson10/lesson10-idd#/title-slide)